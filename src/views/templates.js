...
const messagesTab = `
  <div class="Mt(20px)--ml Mt(16px)--s">
    ${titleGenerator('Messages')}
    ${checkboxGenerator(
      'tinderAutopilotMessages',
      'Send message',
      'Send one of ten messages to your matches'
    )}
    ${Array.from({ length: 10 }, (_, index) =>
      textboxGenerator({
        helpText: `Message ${index + 1}`,
        placeholder: `Your message ${index + 1} to send`,
        className: `messageToSend${index}`,
        defaultValue: defaultMessage
      })
    ).join('')}
  </div>
