...
import MessengerWithCustomMessages from '../automations/MessengerWithCustomMessages';
...
class Sidebar {
  constructor() {
    ...
    this.messengers = new MessengerWithCustomMessages();
  ...
  sidebar = () => {
    ...
    this.infoBanner.innerHTML =
      `<nav class="Pos(r)  H(100%) gamepad-control-off">
        ...
        0,
        0
      )}${autopilot}${massMessage}${messagesTab}${loggerHeader}<div class="txt" style="overflow-y: auto; height: 100%;"></div>` +
      `</div></div></div></nav>`;
  };
  ...
  events = () => {
    ...
    this.bindCheckbox(this.messengers.selector, this.messengers.start, this.messengers.stop);
